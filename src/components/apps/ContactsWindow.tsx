...
const CONTACTS: Contact[] = [
    {
        ...
        Logo: EmailLogo,
        actions: [
            { Icon: Mail,        label: 'Compose',  onClick: () => { window.location.href = MAILTO; } },
            { Icon: Copy,        label: 'Copy',     onClick: () => navigator.clipboard.writeText(PROFILE.email) },
        ...
        fields: [
            { label: 'email', value: PROFILE.email, link: MAILTO, copyValue: PROFILE.email },
    ...
    },
    {
        ...
        name: 'GitHub',
        initials: 'GH',
        subtitle: PROFILE.github.handle,
        gradient: 'linear-gradient(150deg, #2d333b 0%, #1c2128 55%, #0d1117 100%)',
        avatarBg: '#30363d',
        Logo: GitHubLogo,
        actions: [
            { Icon: Star,        label: 'Star',   onClick: () => window.open(PROFILE.github.url, '_blank') },
            { Icon: GitFork,     label: 'Fork',   onClick: () => window.open(PROFILE.github.url, '_blank') },
            { Icon: Code,        label: 'Repos',  onClick: () => window.open(`${PROFILE.github.url}?tab=repositories`, '_blank') },
            { Icon: ExternalLink,label: 'Open',   onClick: () => window.open(PROFILE.github.url, '_blank') },
        ],
        fields: [
            { label: 'profile url', value: `github.com/${PROFILE.github.handle}`, link: PROFILE.github.url, copyValue: PROFILE.github.url },
    ...
    },
    {
        ...
        name: 'LinkedIn',
        initials: 'in',
        subtitle: PROFILE.linkedin.handle,
        gradient: 'linear-gradient(150deg, #1565c0 0%, #0d47a1 55%, #0a2463 100%)',
        avatarBg: '#0a66c2',
        Logo: LinkedInLogo,
        actions: [
            { Icon: UserPlus,    label: 'Connect', onClick: () => window.open(PROFILE.linkedin.url, '_blank') },
            { Icon: MessageSquare, label: 'InMail', onClick: () => window.open(PROFILE.linkedin.url, '_blank') },
            { Icon: ThumbsUp,    label: 'Endorse', onClick: () => window.open(PROFILE.linkedin.url, '_blank') },
            { Icon: ExternalLink,label: 'Open',    onClick: () => window.open(PROFILE.linkedin.url, '_blank') },
        ],
        fields: [
            { label: 'profile url', value: `linkedin.com/in/${PROFILE.linkedin.handle}`, link: PROFILE.linkedin.url, copyValue: PROFILE.linkedin.url },
